...
import { getPayload, type TypedUploadCollection } from "payload";
...
export const generateDataUrl = async (buffer: ArrayBuffer): Promise<string> => {
  try {
    ...
    const blurDataBuffer = await sharp(buffer)
      .resize(width, resizedHeight)
      .blur(blur)
      .toBuffer();
...
export async function migrateBlurDataUrls() {
  ...
  for (const collection of mediaCollections) {
    ...
    for (const mediaItem of docs) {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/${mediaItem.url}`,
      );

      if (response.status !== 200) {
        payload.logger.error(
          `Error fetching ${mediaItem.url}: ${response.statusText}`,
        );
        continue;
      }
