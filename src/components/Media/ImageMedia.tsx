...
export const ImageMedia: React.FC<MediaProps> = (props) => {
  ...
  if (!src && resource && typeof resource === "object") {
    ...
    // biome-ignore lint: yes, we want to assert that these are defined
    width = fullWidth!;
    // biome-ignore lint: yes, we want to assert that these are defined
    height = fullHeight!;
    alt = altFromResource || "";
    focalX = focalXFromResource || 50;
    focalY = focalYFromResource || 50;
    blurDataUrl = blurDataUrlFromResource || undefined;

    const cacheTag = resource.updatedAt;
    const url = height > width ? ogY?.url : ogX?.url;
